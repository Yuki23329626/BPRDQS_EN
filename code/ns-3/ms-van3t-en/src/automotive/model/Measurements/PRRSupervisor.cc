...
namespace ns3 {
  ...
  void
  PRRSupervisor::signalSentPacket(std::string buf,double lat,double lon, uint32_t bufsize)
  {
  ...
  PRRSupervisor::signalReceivedPacket(std::string buf, uint64_t vehicleID)
  {
    ...
    // double curr_throughput_ms = DBL_MAX;
    double curr_payload_size = DBL_MAX;
    ...
    if(m_latency_map.count(buf)>0)
    {
        ...
        // curr_throughput_ms = m_throughput_map[buf]/curr_latency_ms;
        // m_avg_throughput_ms += (curr_throughput_ms-m_avg_throughput_ms)/m_count_latency;
        curr_payload_size = m_throughput_map[buf];
        m_total_payload += curr_payload_size;
