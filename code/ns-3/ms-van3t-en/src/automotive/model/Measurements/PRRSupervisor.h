...
namespace ns3 {
  class PRRSupervisor : public Object {
    ...
    public:
      ...
      void signalSentPacket(std::string buf,double lat,double lon, uint32_t bufsize);
      ...
      double getAverageThroughput(void) {return m_avg_throughput_ms;}
      double getTotalPayload(void) {return m_total_payload;}
    private:
      ...
      std::unordered_map<std::string,int64_t> m_throughput_map;
      int m_count = 0;
      ...
      double m_avg_throughput_ms = 0.0;
      double m_total_payload = 0.0;
