...
main (int argc, char *argv[])
{
  ...
  std::string csv_name="v2i-lte";
  std::string csv_name_cumulative="v2i-lte-cum";
  std::string sumo_netstate_file_name;
  bool print_summary = false;

  /*** 0.b LENA Options ***/
  double interPacketInterval = 1; //default: 100
  bool useCa = false;
  ...
  ***/
  ...
  sumoClient->SetAttribute ("SumoWaitForSocket", TimeValue (Seconds (10.0)));
  ...
  if(m_prr_sup)
    {
      if(csv_name_cumulative!="")
      {
        ...
        else
        {
          ...
          csv_cum_ofstream << "avg_PRR,avg_latency_ms,avg_throughput_byte_ms" << std::endl;
        }

        csv_cum_ofstream << "," << prrSup->getAveragePRR () << "," << prrSup->getAverageLatency () << "," << prrSup->getTotalPayload () << std::endl;
      }
      std::cout << "Average PRR: " << prrSup->getAveragePRR () << std::endl;
      std::cout << "Average latency (ms): " << prrSup->getAverageLatency () << std::endl;
      std::cout << "Average throughput (byte/ms): " << prrSup->getTotalPayload ()/(100*1000) << std::endl;
    }
